...
class WCCR_Cart_Receipt {
	...
	public function __construct( $sid = '' ) {
		global $offset, $wc_cart_reports_options;
		$offset    = $this->get_timezone_offset();
		$this->sid = $sid;
		if ( isset( $wc_cart_reports_options['logip'] ) ) {
	...
	public function set_owner( $owner ) {
		...
		if ( $this->post_author > 0 ) {
			...
			$this->post_author = 0;
			$this->post_title  = __( "Guest's Cart", 'wc-cart-reports' );
		}
	}

    /**
     * Check is guest Order
     * 
     * @since 1.0.0
     *
     * @return bool
	...
	public function load_receipt( $post_id ) {
		...
		$this->ip_address       = get_post_meta( $this->post_id, 'wccr_ip_address', true );
		$this->last_updated     = absint( get_post_meta( $this->post_id, 'wccr_last_updated', true ) ) + ( $offset * 3600 );
		$this->last_connected   = $post->post_date;
		$this->created          = get_post_meta( $this->post_id, 'wccr_cart_created', true );
		$this->sid = get_post_meta( $this->post_id, 'wccr_cart_session_id', true );
	}
	...
	public function status() {
		...
		$stored_status_obj = wp_get_object_terms( $this->post_id, 'wccr_shop_cart_status', false );
		$stored_status = '';
	...
	public function update_receipt() {
		...
		update_post_meta( $this->post_id, 'wccr_cartitems', $this->add_titles_to_cart_items( $this->products ) );
		$post_updated = array(
			'ID'          => $this->post_id,
		...
		update_post_meta( $this->post_id, 'wccr_last_updated', time() );
		update_post_meta( $this->post_id, 'wccr_last_updated_date', date( 'Y-m-d H:i:s' ) );
	}
	...
	public function set_last_connected() {
		update_post_meta( $this->post_id, 'wccr_last_connected', time() );
	}
	...
	public function get_age_text( $time = '' ) {
		...
		if ( $created == '' || $connected == '' ) {
			return '<span style="color:lightgray;">' . __( 'Not Available', 'wc-cart-reports' ) . '</span>';
		} else {
			...
			if ( $number_days > 180 ) {
				return __( '> 6 Months', 'wc-cart-reports' );
			}
			...
			if ( $number_days > 1 ) {
				$days_label = __( 'Days', 'wc-cart-reports' );
			} else {
				$days_label = __( 'Day', 'wc-cart-reports' );
			}

			if ( $number_hours > 1 ) {
				$hours_label = __( 'Hours', 'wc-cart-reports' );
			} else {
				$hours_label = __( 'Hour', 'wc-cart-reports' );
			}

			if ( $number_minutes > 1 ) {
				$minutes_label = __( 'Minutes', 'wc-cart-reports' );
			} else {
				$minutes_label = __( 'Minute', 'wc-cart-reports' );
			}

			if ( $number_seconds > 1 ) {
				$seconds_label = __( 'Seconds', 'wc-cart-reports' );
			} else {
				$seconds_label = __( 'Second', 'wc-cart-reports' );
			}
	...
	public function get_order_id() {
		$order_id = get_post_meta( $this->post_id, 'wccr_order_id', true );
		if ( $order_id != '' ) {
			return get_post_meta( $this->post_id, 'wccr_order_id', true );
		} else {
			return false;
		}
	}
	...
	public function save_order_id( $order_id ) {
		...
		update_post_meta( $this->post_id, 'wccr_order_id', $order_id );
	...
	public function insert_receipt() {
		...
		update_post_meta( $this->post_id, 'wccr_cart_session_id', $this->sid );
		// Only save if the user has checked "Log IP" in the setting page.
		if ( isset( $wc_cart_reports_options['logip'] ) ) {
			update_post_meta( $this->post_id, 'wccr_ip_address', $this->ip_address );
			update_post_meta( $this->post_id, 'wccr_user_agent', $this->user_agent );
		}
		...
		// Save created time.
		update_post_meta( $this->post_id, 'wccr_cart_created', time() );
		update_post_meta( $this->post_id, 'wccr_cart_created_date', date( 'Y-m-d H:i:s' ) );
	...
	public function save_conversion() {
		...
		if ( $this->post_id > 0 && is_int( $this->post_id ) ) { // we have already done the guest check at the higher layer.
			...
			wp_set_object_terms( $this->post_id, 'converted', 'wccr_shop_cart_status' );
	...
	public function record_page_view() {
		...
		if ( $this->status() != 'Converted' && $this->post_id != 0 && $this->post_id > 0 ) { // We don't want to save any online info if the cart is converted - because the carts effectively done. This comes into play if the customer browses the site after converting a cart - it'll mess with the reports.
			if ( isset( $wc_cart_reports_options['logip'] ) ) { // Again, only log if this setting is enabled
				update_post_meta( $this->post_id, 'wccr_ip_address', $this->ip_address );
				update_post_meta( $this->post_id, 'wccr_user_agent', $this->user_agent );
			}
			if ( WP_DEBUG == true ) {
	...
	public function print_cart_actions() {
		global $post;
		global $user_info;
		$cart_receipt = new WCCR_Cart_Receipt();
		...
		switch ( $status ) {
			case 'Converted':
				...
				$order_id = '';
				$order_id = get_post_meta( $post->ID, 'wccr_order_id', true );
				if ( $order_id > 0 && $order_id != '' ) {
					$order_post = get_post( $order_id );
					$order_link = get_admin_url( '', 'post.php?post=' . $order_id . '&action=edit' );
					$actions[]  = new WCCR_Cart_Action( $order_link, __( 'View Order', 'wc-cart-reports' ) );
				}
				do_action( 'wccr_carts_action_converted', $actions );
				break;
			...
			case 'Open':
				...
				if ( $cart_receipt->email() != false && $cart_receipt->email() != '' ) {
					...
					$actions[]  = new WCCR_Cart_Action( $email_link, "Email $full_name" );
				}
				do_action( 'wccr_carts_action_open', $actions );
				break;
			...
			case 'Abandoned':
				...
				do_action( 'wccr_carts_action_abandoned' );
				break;
